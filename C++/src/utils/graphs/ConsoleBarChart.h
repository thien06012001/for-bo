...
// This small library is available only on: https://github.com/abthahiahmed/console-bar-chart-library
...
namespace aarsoftx
{
    ...
    public:
        ConsoleBarChart(string title, int size, int count, vector<ConsoleChartData> result)
        {
            ...
            this->title = title;
            this->size = size; // the size of each bar

            this->result = result;
            this->count = count; // steps of the y axis
        ...
        void show()
        {
            ...
            for (auto &r : this->result)
            {
                ...
                for (int w = 0; w < maxGap; w++)
                    cout << " ";
                cout << "| " << r.name << " : " << r.value;
                if (r.value == this->maxNum)
                    cout << " √ (Max)" << endl;
                else
                    cout << endl;
            }
